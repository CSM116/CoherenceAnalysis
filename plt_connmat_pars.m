function plt_connmat_pars(it,gest_list,connmat_grp)   % args: connect_matrix, component
%     figure;
    max_lw = 12;     min_lw = 1.5;
    max_ns = 22;    min_ns = 5;
    k = size(connmat_grp{1,1},1);
    numges = size(connmat_grp,2);
%     tiledlayout(k,size(connmat_grp,2),'Padding','loose','TileSpacing','none');
    ...
    for j=1:k                                       % loop through number of components
        for m=1:numges                              % loop through gestures          
            ...
            h = plot(G,'Layout','subspace','EdgeCData', G.Edges.Weight,...
                'EdgeLabel',G.Edges.Weight,'NodeLabel',{},'EdgeLabel',{},'Edgealpha',0.85);hold on;
%                 'Edgecolor','#0072BD','NodeColor','#0072BD');
            if (j==k&&m==numges)
                cb = colorbar;
                set(cb,'visible','on','Fontsize',11);
%                 cb.Layout.Tile = 'east';
                ite_col = {[0 0 1],[0.8 0 0.8],[0 0.7 1]};
                J = customcolormap([0 1], [1 1 1; ite_col{it}]);
                colormap(flip(J));
