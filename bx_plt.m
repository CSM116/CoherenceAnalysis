function [delta, probs, stats] = bx_plt(bx_metrics,k,VarNames,GestList,titl,a,type)    % a: 0-normal 1-differential / type: t-ttest w-wilcoxon
    bx_CC = bx_metrics{1};
    bx_NS = bx_metrics{2};
    bx_ED = bx_metrics{3};
    delta = cell(k,length(bx_metrics));
    probs = cell(k,length(bx_metrics));
    stats = cell(k,length(bx_metrics));
...
%             subplot(1,k,i);
            nexttile(i)
            if (j==1)
                [delta{i,j},probs{i,j},stats{i,j}] = bx_plt2(bx_CC,i,a,type);
                ti = VarNames(4);
            elseif (j==2)
                [delta{i,j},probs{i,j},stats{i,j}] = bx_plt2(bx_NS,i,a,type);
                ti = VarNames(7);
            elseif (j==3)
                [delta{i,j},probs{i,j},stats{i,j}] = bx_plt2(bx_ED,i,a,type);
                ti = VarNames(8);
            else
                [delta{i,j},probs{i,j},stats{i,j}] = bx_plt2(bx_GE,i,a,type);
                ti = VarNames(5);
            end
            if(i==3)
                set(gca, 'box','off');
